...
MusicGrabber is a WinForms (.NET 10) desktop app using **Blazor Hybrid** for its UI. The WinForms shell hosts a `BlazorWebView` that renders Razor components with HTML/CSS styling.

**Data flow:** Spotify API (metadata) → yt-dlp (YouTube search + download) → local MP3 files → USB device copy.

### UI Layer — Blazor Components (`src/MusicGrabber/Components/`)

- **MainView.razor** — Full app layout: sidebar with collection browser, track list with checkboxes, download/transfer actions, log panel.
- **SettingsDialog.razor** — Modal overlay for configuring Spotify Client ID, download path, yt-dlp/ffmpeg paths, and target USB drive.
- Styles live in `wwwroot/css/app.css` — dark Spotify-themed design.

### Forms (`src/MusicGrabber/Forms/`)

- **MainForm** — Minimal WinForms shell that hosts `BlazorWebView` and registers DI services.
...
- **LoginService** — Bridges Blazor UI to the WinForms LoginForm dialog for OAuth.
...
- UI is Blazor Razor components (`Components/`), styled with plain CSS (`wwwroot/css/app.css`).
- WinForms is only used for the app shell (`MainForm`) and OAuth browser (`LoginForm`). No `.Designer.cs` files.
- Services are registered via DI in `MainForm` and injected into Razor components.
